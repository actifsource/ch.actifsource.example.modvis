...
angular.module('App.Service.StatefullComponentService', [])
    ...
    ************************************************************************************************************/
    .factory('statefullComponentService', ['$rootScope', 'requestHandler', 'fileHandler', 'utility', 'assert', '$errorHandler', '$window', '$settings', '$filter', '$api', '$q', 'animationStrategy', 
    	...
    	/**
    	 * @public isStatefullComponentBinded
    	 * Returns true if the state full component is binded.
    	 * 
    	 * @param skId state full component id
    	 */
    	modVisService.statefullComponentService.isStatefullComponentBinded = function(skId) { 
    		var isBinded = false;
    		angular.forEach(fService.contexts, function(context) {
    			if (utility.equals(context.skId, skId)) {
    				isBinded = true;
    			}
			});
    		return isBinded
