    ...
    ***********************************************************************************************************/
	.factory('$api', ['assert', '$rootScope', '$errorHandler', '$settings', '$q', '$websocket', '$http', '$timeout', '$cacheFactory', '$route', '$templateCache', 
		...
		function openWebsocket() {
			...
			fService.websocket = $websocket('ws://'+location.hostname+':'+getWebsocketPort()+'/'+$settings.websocketPath, option);
			...
			fService.websocket.onClose(function(event) {
				...
				assert.throwReconnectMessage("Websocket is closed: "+'ws://'+location.hostname+':'+getWebsocketPort()+'/'+$settings.websocketPath);
			});
			...
			fService.websocket.onOpen(function() {
				...
				fService.websocketMode = fWebsocketModeEnum.NORMAL;
				assert.consoleLog_Api("'WebSocket' is open "+"ws://"+location.hostname+":"+getWebsocketPort()+"/"+$settings.websocketPath);
			});
		}
