...
angular.module('App.Api.RequestHandler', [])
    ...
    ***********************************************************************************************************/
    .factory('requestHandler', ['utility', 'assert', '$api', '$settings', '$q', 
    	...
    	/**
    	 * @public setRecfileSyncInfo
    	 * Send the new record state to the server.
    	 * 
    	 * @param modulId 
    	 * @param file {any file from the available record file request}
    	 * @param elementSeq current element sequence
    	 */
        modVis.requestHandler.setRecordFielSyncInfo = function(modulId, file, elementSeq) {
        	var request = null;
        	if (utility.isArrayEmpty(modulId)) {
        		request = {recFileSyncInfo: {recfile: file, seq: elementSeq}};
        	} else {
        		request = {recFileSyncInfo: {recfile: file, seq: elementSeq}, modulId: modulId};
        	}
        	$api.send(request);
    	};
        ...
        modVis.requestHandler.setConfig = function() {
        	const groupId = utility.getURLParameter('groupid');
        	var request = (groupId != undefined)? {config: {selectedconfig: $settings.configNumber, syncContextGroupId: groupId}}:
        										  {config: {selectedconfig: $settings.configNumber}};
        	$api.send(request);
    	};
