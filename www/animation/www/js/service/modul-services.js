...
angular.module('App.Service.ModulService', [])
    ...
    ************************************************************************************************************/
    .factory('modulService', ['modulContext', '$rootScope', 'statefullComponentService', 'requestHandler', 'fileHandler', 'utility', 'assert', '$errorHandler', '$settings', '$api', '$q', '$timeout', 
    	 ...
    	 * 		recordContext = {availableRecordFiles:, currentRecordFileIndex: requestedFileType:, loadFileInProgress:, elementStates:, snapshotSeq:, currentElementIndex:, peerRecordFileSyncInfo: {recordFile, elementSeq}}
    	 * 		
    	 */
    	 ...
    	 * 								/elementStates:/, /snapshotSeq:/, peerRecordFileSyncInfo: {recordFile, elementSeq}}
    	 * 
    	...
    	function handleAvailableRecordFileResponse(context, jsonUpdateData) {
    		...
    		// Restore peer record file sync info before clear context.
    		var peerRecordFileSyncInfo = context.recordContext.peerRecordFileSyncInfo
    		modulContext.clearRecordContext(context);
    		context.recordContext.peerRecordFileSyncInfo = peerRecordFileSyncInfo;
    		
    		modulContext.setAvailableRecordFiles(context, availableRecFiles);
    	
    		// Request peer sync file if sync info is available.
    		if (context.recordContext.peerRecordFileSyncInfo == null) {
    			modulContext.requestLastFile(context);    			
    		} else {
    			modulContext.requestPeerSyncFile(context);    			
    		}
    	}
    	...
    	/**
    	 * @private handleModulSyncInfoResponse
    	 * Sync modul with peer moduls in different browsers.
    	 * 
    	 * @param context
    	 * @param jsonUpdateData
    	 */
    	function handleModulSyncInfoResponse(context, jsonUpdateData) {
    		assert.assertNotNull(jsonUpdateData.modulSyncInfo, "'ModulService' Handle modul sync info response is not available.");
    		assert.assertNotNull(jsonUpdateData.modulSyncInfo.mode, "'ModulService' Handle modul sync info response 'mode' is not available.");
    		
    		// Sync live mode
    		if (jsonUpdateData.modulSyncInfo.mode == modulContext.Enum.MODE.LIVE) {
    			if (context.currentMode != modulContext.Enum.MODE.LIVE)
        		{
        		    modVisService.modulService.play(context.modulId);
        		}
    		...
    		// Sync record mode
    		if (jsonUpdateData.modulSyncInfo.mode == modulContext.Enum.MODE.RECORD) {
        		assert.assertNotNull(jsonUpdateData.modulSyncInfo.recFileSyncInfo.recfile, "'ModulService' Handle modul sync info response 'recordFileSyncInfo.recfile' is not available.");
				assert.assertNotNull(jsonUpdateData.modulSyncInfo.recFileSyncInfo.seq, "'ModulService' Handle modul sync info response 'recordFileSyncInfo.seq' is not available.");
				assert.assertNotNull(jsonUpdateData.modulSyncInfo.recFileSyncInfo.reload, "'ModulService' Handle modul sync info response 'recordFileSyncInfo.reload' is not available.");
				
				// Check is record mode in sync
				if (context.currentMode != modulContext.Enum.MODE.RECORD) {
					modulContext.setPeerRecordFielSyncInfo(context, jsonUpdateData.modulSyncInfo.recFileSyncInfo.recfile, jsonUpdateData.modulSyncInfo.recFileSyncInfo.seq);
        		    modVisService.modulService.pause(context.modulId);
        		    return;
        		}
				
				// Check if the record context is initialized
				if (!modulContext.initRecordContextComplete(context)) {
					modulContext.setPeerRecordFielSyncInfo(context, jsonUpdateData.modulSyncInfo.recFileSyncInfo.recfile, jsonUpdateData.modulSyncInfo.recFileSyncInfo.seq);
					requestHandler.getAvailableRecordFile(modulId);
        		    return;
				}
				
        		// Check if record file in sync
    			if (jsonUpdateData.modulSyncInfo.recFileSyncInfo.reload || modulContext.getCurrentRecordFile(context) != jsonUpdateData.modulSyncInfo.recFileSyncInfo.recfile) {
    				modulContext.setPeerRecordFielSyncInfo(context, jsonUpdateData.modulSyncInfo.recFileSyncInfo.recfile, jsonUpdateData.modulSyncInfo.recFileSyncInfo.seq);
    				modulContext.requestPeerSyncFile(context);
        		    return;
    		    }
				
				// Check if the element index is in sync
				if (modulContext.getCurrentElementSeq(context) != jsonUpdateData.modulSyncInfo.recFileSyncInfo.seq) {
					modulContext.setPeerRecordFielSyncInfo(context, jsonUpdateData.modulSyncInfo.recFileSyncInfo.recfile, jsonUpdateData.modulSyncInfo.recFileSyncInfo.seq);
					modulContext.setCurrentElementIndex(context, modulContext.getPeerSyncRecordElementStateIndex(context));
					updateRecordSnapshot(context);
					return;
				}
    			
    			// Is in sync with peer
    		}
    	}
    	...
    	/**
    	 * @private isModulUpdateSyncInfoResponse
    	 * 
    	 * @param context
    	 * @param jsonResponseData
    	 */
    	function isModulSyncInfoResponse(context, jsonResponseData) {
    		return jsonResponseData.modulSyncInfo != null;
    	}
    	...
    	function isModulServiceResponse(jsonResponseData) {
    		return jsonResponseData.recfiles != null || 
    		       ...
    		       jsonResponseData.modulSyncInfo != null;
    	}
    	...
    	function websocketDataListener(jsonResponseData) {
    		synchronizeFunction(function () {
    			...
    			/* isModulUpdateSyncInfoResponse */
    			if (isModulSyncInfoResponse(context, jsonResponseData)) {
    				assert.consoleLog_Service("'ModulService' Handle modul sync info response");
    				handleModulSyncInfoResponse(context, jsonResponseData);
    				responseDataHandled = true;
    			}
